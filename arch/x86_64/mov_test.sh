...
test_mov_reg_u32()
{
	...
	local expected=$(asm_hex<<<"mov \$$2, %$1");
	if [ "${result,,}" != "${expected,,}" ]; then
		echo "test_mov_reg_u32 ERROR: given [$c] got [$result] but expected [$expected]";
	fi;
}
test_mov_reg_ptrreg()
{
...
test_mov_ptrreg_reg()
{
	...
	local v=$(echo $r| tr "," " " | tr -s " " | tr " " "\t");
	local code=$(cut -f2<<<$v);
	expected=$(asm_hex<<<"mov %$1, (%$2)");
	if [ "$got" != "$expected" ]; then
		echo "ERROR: given [$c] got [$code] but expected [$expected]";
	fi;
}
...
set_r1()
{
	...
	test_mov_reg_u32 $v1 $(( RANDOM % 256 ));
}
